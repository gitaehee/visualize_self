...
const MovieBarChart = () => {
  ...
  useEffect(() => {
    axios.get('http://127.0.0.1:5000/api/top10movies')
      .then(response => {
        const movies = typeof response.data === 'string' ? JSON.parse(response.data) : response.data;
  ...
  const customColors = [
    "#4a69bd", "#6a89cc", "#78e08f", "#38ada9", "#e55039", 
    "#f6b93b", "#fa983a", "#b71540", "#60a3bc", "#82ccdd"
  ];
  ...
  return (
    <div style={{ height: '500px', width: '800px', backgroundColor: "#222", padding: "20px", borderRadius: "10px" }}>
      <ResponsiveBar
        data={data.length > 0 ? data : [{ movie_name: "데이터 없음", audience_count: 0 }]}
        keys={['audience_count']}
        indexBy="movie_name"
